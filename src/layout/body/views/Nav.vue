...
import { delete_token } from '../../../util/ts/token'
import LoginVue from '../../../components/Login.vue'
import { createDOM } from '../../../util/ts/create'
...
export default defineComponent({
  setup() {
    ...
    const { user, loginState } = storeToRefs(store)
    return { user, loginState, store }
  },
  data() {
    return {
      userCardShow: false,
      outImg: false,
      outCard: true,
      nav_user_photo: this.$refs.nav_user_photo as any,
      user_card: this.$refs.user_card as any
    }
  },
  methods: {
    ...
    routerPush(url: string) {
      if (url === '/user/personal/center/') url += this.user.id
    ...
    userCardAddAction() {
      this.nav_user_photo.classList.add('action')
      this.user_card.classList.add('action')
      this.nav_user_photo.addEventListener('mouseleave', this.imgMouseout)
      this.user_card.addEventListener('mouseover', this.userCardMouseenter)
      this.user_card.addEventListener('mouseleave', this.userCardMouseout)
      requestAnimationFrame(this.animationCallback)
    },
    userCardDeleteAction() {
      this.outImg = false
      this.outCard = true
      this.nav_user_photo.classList.remove('action')
      this.user_card.classList.remove('action')
      this.nav_user_photo.removeEventListener('mouseleave', this.imgMouseout)
      this.user_card.removeEventListener('Mouseleave ', this.userCardMouseout)
      this.user_card.removeEventListener('mouseover', this.userCardMouseenter)
    ...
    logout() {
      this.nav_user_photo.removeEventListener('mouseover', this.imgMouseover)
  ...
  mounted() {
    ...
    if (this.loginState)
      this.nav_user_photo.addEventListener('mouseover', this.imgMouseover)
  },
  computed: {
    loginStatus() {
      return false
